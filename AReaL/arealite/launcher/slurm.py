...
class SlurmLauncher:
    ...
    def submit(self, job_name, cmd, **kwargs):
        ...
        """
        return self.submit_array(
            job_name, cmd, count=1, **kwargs, exclude="slurmd-[91-94]"
        )
...
def slurm_main(run_id: int = 0):
    ...
    try:
        ...
        if isinstance(e, JobException):
            ...
            print(
                f">>>>>>>>>>>>>>>>>>>>>>> recover_this={recover_this} reason {e.reason} run_id {config.run_id} mode {config.recover.mode} >>>>>>>>>>>>>>>>>>>>>>>>>",
                flush=True,
            )
            if recover_this:
                slurm_main(run_id=run_id + 1)
        raise e
