...
from arealite.utils.recover import check_if_recover
...
class RayLauncher:
    def __init__(self, experiment_name: str, trial_name: str, fileroot: str):
        ...
        self.placement_group_jobs = defaultdict(list)
    ...
    def submit(
        ...
        if kwargs is None:
            kwargs = {}
        if env_vars is not None and "TRITON_CACHE_DIR" in env_vars:
            triton_path = env_vars.pop("TRITON_CACHE_DIR")
            env_vars["TRITON_CACHE_DIR"] = os.path.join(triton_path, str(uuid.uuid4()))
        ...
        if placement_group is not None:
            self.placement_group_jobs[job_array_name].append(job_name)
        return future
    ...
    def submit_array(
        ...
        self.placement_groups[job_array_name] = placement_group
        try:
    ...
    def stop(self, job_name: str, force: bool = False):
        ...
        if len(job_name.split(":")) > 1:
            job_array_name = job_name.split(":")[0]  # Use the job array name
        ...
        if job_name in self.jobs:
            ...
            if job_array_name:
                ...
                assert job_array_name in self.placement_groups
                assert job_name in self.placement_group_jobs[job_array_name]
                self.placement_group_jobs[job_array_name].remove(job_name)

                if len(self.placement_group_jobs[job_array_name]) == 0:
                    ray.util.remove_placement_group(
                        self.placement_groups[job_array_name]
                    )
                    time.sleep(1)
                    self.placement_groups.pop(job_array_name)
...
def ray_main(config, run_id: int = 0):
    ...
    try:
        ...
        if isinstance(e, JobException):
            ...
            logger.info(
                f">>>>>>>>>>>>>>>>>>>>>>> recover_this={recover_this} reason {e.reason} run_id {run_id} mode {config.recover.mode} >>>>>>>>>>>>>>>>>>>>>>>>>"
            )
            if recover_this:
