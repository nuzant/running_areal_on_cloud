...
from arealite.utils.launcher import get_env_vars
...
LOCAL_WAIT_CHECK_TIME_INTERVAL = 5
TORCHRUN_ERROR_OUTPUT = (
    ...
    def wait(
        ...
        while len(left) > 0:
            ...
            for job_name in list(left):
                ...
                # )
                status = check_process_status(job)
            ...
            time.sleep(LOCAL_WAIT_CHECK_TIME_INTERVAL)
...
def local_main(config, run_id: int = 0):
    ...
    if not config.server_only:
        launcher.submit(
            job_name="trainer",
            cmd=f"torchrun --max_restarts 0 --nnodes 1 --nproc-per-node {alloc_mode.train_world_size} --master-addr localhost --master-port {find_free_ports(1, (10000, 50000))[0]} {' '.join(sys.argv[1:])}",
